"""
Diabetes Risk Assessment - PostgreSQL Schema Migration
Barcha jadvallarni yaratadi va sample ma'lumotlar qo'shadi.

Ishlatish:
    pip install psycopg2-binary python-dotenv
    python diabetes_db_migration.py

.env fayli (ixtiyoriy):
    DB_HOST=localhost
    DB_PORT=5432
    DB_NAME=diabetes_risk
    DB_USER=postgres
...
-- 10. Feature contributions (SHAP values)
...
CREATE INDEX IF NOT EXISTS idx_test_results_biomarker ON test_results(biomarker_id);
...
CREATE INDEX IF NOT EXISTS idx_run_logs_assessment    ON model_run_logs(risk_assessment_id);
CREATE INDEX IF NOT EXISTS idx_feature_run            ON feature_contributions(run_log_id);
CREATE INDEX IF NOT EXISTS idx_notifications_patient  ON notifications(patient_id);
CREATE INDEX IF NOT EXISTS idx_notifications_unsent   ON notifications(patient_id) WHERE is_sent = FALSE;
...
def ensure_database_exists():
    """Agar database mavjud bo'lmasa, uni yaratadi."""
    config = {**DB_CONFIG, "dbname": "postgres"}
    conn = None
    try:
        ...
        if not cur.fetchone():
            ...
            print(f"[OK] Database yaratildi: {DB_CONFIG['dbname']}")
        else:
            print(f"[OK] Database mavjud: {DB_CONFIG['dbname']}")
        cur.close()
    except Exception as e:
        print(f"[XATO] Database yaratishda muammo: {e}")
        sys.exit(1)
    finally:
        if conn:
            conn.close()
...
def run_migration(conn, label: str, sql_block: str):
    """Bitta SQL blokini ishga tushiradi va natijasini chiqaradi."""
    try:
        cur = conn.cursor()
...
def print_summary(conn):
    """Yaratilgan jadvallar sonini chiqaradi."""
    cur = conn.cursor()
    cur.execute("""
        SELECT table_name,
    ...
    print("\n  Jadval nomi                  Ustunlar")
    print("  " + "-" * 40)
    for name, cols in rows:
        print(f"  {name:<30} {cols}")
    print(f"\n  Jami: {len(rows)} ta jadval")


def main():
    print("=" * 52)
    print("  Diabetes Risk DB — Migration")
    print("=" * 52)

    # 1. Database borligini tekshir / yarat
    print("\n[1/4] Database tekshirilmoqda...")
    ensure_database_exists()

    # 2. Ulaning
    print("\n[2/4] Ulaning...")
    conn = get_connection()
    print(f"  [OK] {DB_CONFIG['host']}:{DB_CONFIG['port']}/{DB_CONFIG['dbname']}")

    # 3. UUID extension
    print("\n[3/4] Extensions yoqilmoqda...")
    run_migration(conn, "pgcrypto (UUID)", 'CREATE EXTENSION IF NOT EXISTS "pgcrypto";')

    # 4. Sxema
    print("\n[4/4] Sxema yaratilmoqda...")
    run_migration(conn, "Enum turlari",      ENUM_TYPES)
    run_migration(conn, "Jadvallar (11 ta)", TABLES)
    run_migration(conn, "Indekslar (12 ta)", INDEXES)
    run_migration(conn, "Seed ma'lumotlar",  SEED_DATA)
